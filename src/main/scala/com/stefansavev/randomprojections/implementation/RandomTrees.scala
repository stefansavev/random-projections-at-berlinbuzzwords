...
class RandomTrees(val dimReductionTransform: DimensionalityReductionTransform,
                  val reportingDistanceEvaluator: ReportingDistanceEvaluator,
                  val signatureVecs: SignatureVectors,
                  val datasetSplitStrategy: DatasetSplitStrategy,
                  val header: ColumnHeader,
                  val invertedIndex: Index,
                  val trees: Array[RandomTree]){
}
