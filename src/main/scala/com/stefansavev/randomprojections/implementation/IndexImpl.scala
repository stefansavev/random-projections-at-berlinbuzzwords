...
import com.stefansavev.randomprojections.serialization.{BinaryFileSerializer, PointSignaturesSerializer}
  ...
  def fillCountsWithoutWeights(query: Array[Double], settings: SearcherSettings, trainingSet: DataFrameView, bucketIds: Array[Int], pointCounts: Array[Int], maxIndex: Int, countsOfCounts: Array[Int]): Unit = {
    ...
    while (i < leafEnd) {
      ...
      while (j < pointsEnd) {
        ...
        val inc = if (prevCount == 0) {
          1 + signatures.overlap(querySigs, pointId)
        } else 1
